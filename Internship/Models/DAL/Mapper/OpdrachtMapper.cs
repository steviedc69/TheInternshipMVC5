...
namespace Internship.Models.DAL.Mapper
{
    public class OpdrachtMapper : EntityTypeConfiguration<Opdracht>
    {
        public OpdrachtMapper()
        {
            ...
            HasOptional(o => o.Ondertekenaar).WithMany().Map(m => m.MapKey("Ondertekenaar")).WillCascadeOnDelete(false);
            HasOptional(o => o.StageMentor).WithMany().Map(m => m.MapKey("StageMentor")).WillCascadeOnDelete(false);
